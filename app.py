...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        sid = addStudent("Rachel", "Waring", "rachelwaring@example.com", "2025-09-01")
        print(f"Added id={sid}")

        print("\nUpdating that student's email...")
        updateStudentEmail(sid, "rachelwaring317@example.com")
        print("After update:", [s for s in getAllStudents() if s["student_id"] == sid])

        print("\nDeleting the sample student...")
        deleteStudent(sid)
        print("Final rows:")
        print(getAllStudents())
        sys.exit(0)
